...
async function fetchStockAnalytics() {
  const data = await fetchAdminJson("/api/admin/analytics/stock", {
    method: "GET"
  });
...
function renderStockAnalyticsSummary(summary = {}) {
  const totalStock = document.getElementById("stockAnalyticsTotalStock");
  const activeBikes = document.getElementById("stockAnalyticsActiveBikes");
  const lowStock = document.getElementById("stockAnalyticsLowStock");
  const outOfStock = document.getElementById("stockAnalyticsOutOfStock");
...
function renderStockMovementChart(movements = []) {
  const chart = document.getElementById("stockAnalyticsMovementChart");
  ...
  chart.innerHTML = `
    ...
    >
      ...
      <text class="stock-chart-subtitle" x="${paddingLeft}" y="34">
...
function renderStockMovementList(movements = []) {
  const list = document.getElementById("stockAnalyticsMovementList");
...
async function loadStockAnalytics() {
  const movementList = document.getElementById("stockAnalyticsMovementList");
  ...
  try {
    ...
    renderStockAnalyticsSummary(data.summary || {});
renderStockMovementChart(data.dailyMovements || []);
renderStockMovementList(data.dailyMovements || []);
  } catch (error) {
